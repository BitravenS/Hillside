...
func (cli *Client) validateChatMessageIntegrity(env *models.Envelope, msg *models.ChatMessage) error {
	if msg == nil {
		return ErrValidationIssue.WithDetails("Chat message cannot be nil")
	}
	if string(msg.Ciphertext) == "" {
		return ErrValidationIssue.WithDetails("Chat message content cannot be empty")
	}

	if len(msg.Ciphertext) > 10000 { // Arbitrary limit for message length
		return ErrValidationIssue.WithDetails("Chat message content exceeds maximum length of 10000 characters")
	}
	if env.Timestamp == 0 {
...
func (cli *Client) validateMessageSecurity(env *models.Envelope, strSenderID string) error {
	if env.Sender.PeerID != strSenderID {
		return ErrSecurityIssue.WithDetails("Sender ID does not match the message sender")
	}
	if env.Timestamp > time.Now().UnixMicro() {
		return ErrSecurityIssue.WithDetails(fmt.Sprintf("Message timestamp %d is in the future, now is %d", env.Timestamp, time.Now().UnixMicro()))
	}

	verify := crypto.ValidateSignature(env.Sender.DilithiumPub, env.Payload, env.Signature)
	if verify != nil {
		return ErrSecurityIssue.WithDetails(verify.Error())
	}
